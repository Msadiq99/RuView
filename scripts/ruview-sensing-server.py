...
class Handler(BaseHTTPRequestHandler):
    ...
    def _json(self, code: int, body: dict) -> None:
        ...
        # Allow simple cross-origin access for local network dashboards
    ...
    def do_GET(self) -> None:
        ...
        # Simple in-server dashboard HTML for same-origin browser viewing
        if path == "/":
            html = """
<!doctype html>
<html>
<head><meta charset="utf-8"><title>RuView Sensing Dashboard</title></head>
<body>
<h2>RuView Sensing Dashboard</h2>
<div id="status">Connecting...</div>
<pre id="payload">-</pre>
<script>
async function fetchLatest(){
  try{
    const r = await fetch('/api/v1/sensing/latest');
    if(!r.ok){ document.getElementById('status').textContent = 'No data'; return; }
    const j = await r.json();
    document.getElementById('status').textContent = `node: ${j.node_id} — ${j.n_persons} person(s) — confidence ${j.confidence}`;
    document.getElementById('payload').textContent = JSON.stringify(j, null, 2);
