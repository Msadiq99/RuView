...
def main():
    ...
    # Background writer: dump hub.compute() to the feature JSON file
    feature_file = os.environ.get("RUVIEW_FEATURE_JSON", "/tmp/ruview-last-feature.json")

    def feature_writer(hub, path, interval, stop_ev):
        dirn = os.path.dirname(path) or "/tmp"
        while not stop_ev.is_set():
            try:
                d = hub.compute()
                payload = {
                    "node_id": getattr(d, "get", lambda k, default=None: None)("node_id") if False else "demo-node-1",
                    "ts": time.time(),
                    "timestamp_ms": int(time.time() * 1000),
                    "presence": bool(d.get("presence", False)),
                    "n_persons": int(d.get("presence", False)),
                    "confidence": float(d.get("coh_csi", 0.0) or d.get("coh_mw", 0.0)),
                    "motion": float(d.get("motion", 0.0)),
                    "breathing_rate_bpm": d.get("br"),
                    "heartrate_bpm": d.get("hr"),
                    "privacy_class": 2,
                }
                # atomic write
                tf = tempfile.NamedTemporaryFile(mode="w", dir=dirn, delete=False)
    ...
    threading.Thread(target=feature_writer, args=(hub, feature_file, 1.0, stop), daemon=True).start()
